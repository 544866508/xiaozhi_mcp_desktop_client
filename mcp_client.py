...
class Api:
    ...
    def start_mcp(self):
        ...
        # 复制主进程完整环境变量，子进程完全复用当前环境
        child_env = os.environ.copy()
        # 可选：如需自定义覆盖环境变量在这里追加
        # child_env["MCP_DEBUG"] = "1"

        # 当前项目工作目录，让子进程工作目录和主进程保持一致
        work_dir = os.getcwd()
        ...
        if getattr(sys, 'frozen', False):
            ...
            # 开发环境：固定使用当前运行这套Python，并且传入完整环境
            cmd = [sys.executable, "mcp_pipe.py"]

        print(f"🚀 正在拉起独立后台进程: {' '.join(cmd)}")
        # 关键：传入 env=child_env，显式绑定主进程环境
        self.proc = subprocess.Popen(
            cmd,
            env=child_env,
            cwd=work_dir,
            # Windows 可选：解决控制台弹窗、编码问题
            text=True,
            encoding="utf-8"
        )
